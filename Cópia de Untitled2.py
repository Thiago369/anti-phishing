...
def check_ssl_certificate(url):
    ...
    try:
        ...
        score += 30
        reasons.append("Risco ALTO: Falha ao conectar (mesmo com HTTPS), possível problema de rede ou domínio falso.")
...
# ### Exemplo de `app.py` para Streamlit
# 
# Este é o código que você colocaria no seu arquivo `app.py` no GitHub. Ele substitui a interação via `input()` e `print()` por componentes do Streamlit (`st.text_input`, `st.button`, `st.write`, `st.markdown`).
...
from urllib.parse import urlparse
import requests
import whois # Certifique-se de que python-whois está instalado
import tldextract
import validators # Certifique-se de que validators está instalado
...
# --- Helper Functions (as defined in your notebook) ---
...
st.set_page_config(page_title="Analisador de Risco de Phishing de URL", layout="centered")

st.title("🕵️‍♂️ Analisador de Risco de Phishing de URL")
st.markdown("Digite uma URL para avaliar seu risco de phishing. **Lembre-se: esta é uma ferramenta auxiliar, não uma garantia.**")

user_url = st.text_input("Por favor, digite a URL para análise:", "https://www.example.com")

if st.button("Analisar URL"):
    if user_url:
        with st.spinner('Analisando URL... Por favor, aguarde.'):
            score, reasons = calculate_phishing_score(user_url)
            risk_level = get_risk_level(score)
            recommendation = get_final_recommendation(score)
        ...
        st.write(f"**Nível de Risco:** <span style='color: {'red' if risk_level == 'ALTO' else ('orange' if risk_level == 'MÉDIO' else 'green')}; font-weight: bold;'>{risk_level}</span>", unsafe_allow_html=True)

        st.subheader("--- Motivos Detectados ---")
        if reasons:
            for reason in reasons:
        ...
        st.warning("Por favor, digite uma URL para analisar.")
...
def check_ssl_certificate(url):
    ...
    try:
        ...
        score += 50
        reasons.append("Risco ALTO: Erro no certificado SSL (inválido ou auto-assinado)."+ str(e))
    except requests.exceptions.ConnectionError as e:
        score += 30
        reasons.append("Risco ALTO: Falha ao conectar (mesmo com HTTPS), possível problema de rede ou domínio falso." + str(e))
    except requests.exceptions.Timeout:
        score += 15
        reasons.append("Risco MÉDIO: Requisição HTTPS expirou, possível servidor lento ou inexistente.")
    except Exception as e:
...
# Crie um arquivo chamado `requirements.txt` na **raiz** do seu repositório GitHub e adicione o seguinte conteúdo:
